...
対象とするフォルダは`main/`配下、Git運用は原則ユーザーが実施します。
...
## 6. AI支援開発環境（MCP API）の利用
本プロジェクトでは、AI（Antigravity等）とUnityエディタを連携させるため **MCP (Model Context Protocol) for Unity** を導入・運用しています。
*   AIはMCPを通じて、シーン内のオブジェクト検索、コンポーネントのプロパティ読み書き、Udonスクリプトへの軽微な編集などを直接・または半自動的に支援することが可能になります。
*   接続トラブルを防ぐため、通信プロトコル（Transport）は常に **`STDIO`** を使用してください。詳細は `docs/Unity_MCP接続手順.md` を参照してください。
